...
def _refresh_access_token() -> str:
    ...
    if resp.status_code != 200:
        raise RuntimeError(f"refresh_token flow failed: HTTP {resp.status_code}: {resp.text[:500]}")
...
def main() -> int:
    ...
    if not token:
        print(
            "ERROR: Missing Pinterest token. "
            "Set refresh-token secrets (PINTEREST_CLIENT_ID_EQUALLE, PINTEREST_CLIENT_SECRET_EQUALLE, PINTEREST_REFRESH_TOKEN_EQUALLE) "
