# ============================================
# File: instagram_reels/comment_worker.py
# Purpose: Post a single "first comment" under the most recent successfully published IG Reel,
#          using the shared instagram_reels post state file.
# Notes:
# - Separate from posting workflow: comment is delayed and optional.
...
STATE_REL_PATH = Path("instagram_reels/state/instagram_reels_post_state.json")
...
TEMPLATES = [
    ...
    "If the paper starts loading up, wipe/rinse it and keep going—clean paper cuts cleaner. Does it clog on your surface?",
    ...
    "If you’re getting random deep lines, the sheet may be loaded or folded. Are you using fresh paper?",
    "Before the next coat, clean the surface well so dust doesn’t telegraph through. Do you wipe down between coats?",
]


def repo_root_from_this_file() -> Path:
    # instagram_reels/comment_worker.py -> repo root = two levels up
...
def stable_rng(seed_text: str) -> random.Random:
    """Deterministic RNG per seed_text (stable decisions across retries)."""
...
def find_latest_success_to_comment(state: Dict[str, Any]) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    """Returns (run_entry, item_record) for latest successful run eligible for commenting."""
    ...
    for run in reversed(runs):
        ...
        media_id = (run.get("media_id") or "").strip()
...
def post_comment(media_id: str, token: str, version: str, message: str) -> str:
    """POST /{ig-media-id}/comments (requires instagram_manage_comments). Returns comment_id."""
    url = f"{graph_base(version)}/{media_id}/comments"
...
def main() -> int:
    ...
    if not token:
        print("ERROR: Missing IG_ACCESS_TOKEN (fallbacks: INSTAGRAM_ACCESS_TOKEN, META_ACCESS_TOKEN, FB_PAGE_TOKEN)")
        return 2
    ...
    media_id = str(run.get("media_id") or "").strip()
    ...
    # Deterministic per (media_id + UTC day) so retries won't change delay.
    try:
        jitter_max = int((os.getenv("IG_REELS_COMMENT_JITTER_MAX_SEC") or str(DEFAULT_JITTER_MAX_SEC)).strip())
    except Exception:
    ...
    if jitter_max > 0:
        jitter_seed = f"{media_id}|{utc_today()}"
        jrng = stable_rng(jitter_seed)
        delay = jrng.randint(0, jitter_max)
        if delay > 0:
            print(f"[comment_worker] jitter sleep: {delay}s (max={jitter_max}s) media_id={media_id}")
            time.sleep(delay)
    # -------------------------------------------------------------

    # Determine (or reuse) plan for this media (stable across retries)
    ...
    if plan.get("decision") in ("comment", "skip"):
        ...
        rng = stable_rng(media_id or video_url)
    ...
    if decision == "skip":
        ...
        print(f"SKIP: Policy skip (10%). media_id={media_id} manifest={manifest}")
        return 0
    ...
    dry_run = (os.getenv("IG_REELS_COMMENT_DRY_RUN") or os.getenv("DRY_RUN") or "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
    if dry_run:
        ...
        items[video_url] = rec2
        save_json_atomic(state_path, state)
        print(f"DRY RUN: would comment on media_id={media_id}: {msg}")
        return 0

    try:
        cid = post_comment(media_id=media_id, token=token, version=version, message=msg)
        ...
        print(f"OK: Commented on IG Reel. media_id={media_id} comment_id={cid} manifest={manifest}")
        return 0
